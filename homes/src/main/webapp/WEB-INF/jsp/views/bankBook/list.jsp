		...
		if (result.status) {
			...
			for (var i = 0; i < listLength; i++) {
				...
				if (thisData.bankBookType) {
					...
					html += '	<td class="text-center' + (mainType ? ' color-orange font-bold' : '') + '">' + (thisData.bankBookType.typeName ? thisData.bankBookType.typeName : '') + '</td>';
				} else {
					html += '	<td class="text-center"></td>';
				}
					html += '	<td class="text-center">' + thisData.member.memberName + '</td>';
					html += '	<td>' + common.string.toEmpty(thisData.bankBookName) + '</td>';
					html += '	<td class="text-center">' + common.string.toEmpty(thisData.bank.bankName) + '</td>';
					html += '	<td>' + common.string.toEmpty(thisData.acountNumber) + '</td>';
					html += '	<td class="text-right color-green">' + (thisData.totalAmount ? common.number.addComma(thisData.totalAmount) : 0) + '</td>';
					html += '	<td class="text-center">' + common.date.toString(new Date(list[i].openDate), '-') + '</td>';
					html += '	<td class="text-center">' + (list[i].closeDate ? common.date.toString(new Date(list[i].closeDate), '-') : '') + '</td>';
					html += '	<td class="text-center"><button type="button" class="btn bt-default btn-sm btn_bankBook_detail" idx="' + thisData.bankBookIdx + '">상세보기</button></td>';
					html += '</tr>';
	...
	// 결과
		...
		if (result.status) {
			...
			for (var i = 0; i < bankLength; i++) {
				if (type == 'S') {
					if (i == 0) {
						html += '<label class="checkbox-inline"><input type="checkbox" checked value="all" class="bank_all_check"/>전체</label>';
					}
					
					html += '<label class="checkbox-inline">';
					html += '	<input type="checkbox" checked value="' + bankList[i].bankIdx + '" class="bank_data_check"/>' + bankList[i].bankName;
					html += '</label>';
...
<div>
	<div class="search-form panel panel-default">
		...
		<table class="table">
			...
			<tbody>
				...
				<tr>
					<th>개설일</th>
					<td>
						<input type="text" class="form-control datepicker" id="startDateOpen"/>
					</td>
					<td>
						<input type="text" class="form-control datepicker" id="endDateOpen"/>
					</td>
					<th>만기일</th>
					<td>
						<input type="text" class="form-control datepicker" id="startDateClose"/>
					</td>
					<td>
						<input type="text" class="form-control datepicker" id="endDateClose"/>
					</td>
					<td></td>
					<td></td>
				</tr>
	...
	<div class="top-button-area">
		...
		<button class="btn btn-primary float-right" id="insertModalOpenBtn" data-toggle="modal" data-target="#insertModal">등록</button>
	</div>
	<table id="bankBookTable" class="table table-responsive-md">
		<colgroup>
			<col width="3%"/>
			<col width="7%"/>
			<col width="5%"/>
	...
	<div class="modal-dialog modal-sm" role="document">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myInsertModalLabel">등록</h4>
			</div>
			<div class="modal-body">
				<table class="table">
					<tr>
						<th>은행</th>
						<td>
							<select id="bankSelect" class="form-control"></select>
						</td>
					</tr>
					<tr>
						<th>통장명</th>
						<td>
							<input type="text" class="form-control" id="bankBookName"/>
						</td>
					</tr>
					<tr>
						<th>계좌번호</th>
						<td>
							<input type="text" class="form-control" id="acountNumber"/>
						</td>
					</tr>
					<tr>
						<th>통장 유형</th>
						<td>
							<select id="bankBookTypeSelect" class="form-control"></select>
						</td>
					</tr>
					<tr>
						<th>개설일</th>
					...
					<tr>
						<th>만기일</th>
						<td>
							<input type="text" class="form-control datepicker" id="closeDate"/>
						</td>
					</tr>
				</table>
			</div>
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-default" id="insertCloseBtn" data-dismiss="modal">닫기</button>
			</div>
		</div>
	</div>
</div>
	...
	<div class="modal-dialog modal-sm" role="document">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myUpdateModalLabel"></h4>
			</div>
			<div class="modal-body">
				...
				<table class="table">
					<tr>
						<th>은행</th>
						<td>
							<select id="updateBankSelect" class="form-control"></select>
						</td>
					</tr>
					<tr>
						<th>통장명</th>
						<td>
							<input type="text" class="form-control" id="updateBankBookName"/>
						</td>
					</tr>
					<tr>
						<th>계좌번호</th>
						<td>
							<input type="text" class="form-control" id="updateAcountNumber"/>
						</td>
					</tr>
					<tr>
						<th>통장 유형</th>
						<td>
							<select id="updateBankBookTypeSelect" class="form-control"></select>
						</td>
					</tr>
					<tr>
						<th>금액</th>
						<td>
							<input type="text" class="form-control text-right" id="updateTotalAmount"/>
						</td>
					</tr>
					<tr>
						<th>개설일</th>
						<td>
							<input type="text" class="form-control datepicker" id="updateOpenDate"/>
						</td>
					</tr>
					<tr>
						<th>만기일</th>
						<td>
							<input type="text" class="form-control datepicker" id="updateCloseDate"/>
						</td>
					</tr>
				</table>
			</div>
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-default" id="updateCloseBtn" data-dismiss="modal">닫기</button>
			</div>
		</div>
	</div>
</div>