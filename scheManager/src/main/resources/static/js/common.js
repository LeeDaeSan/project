...
var converter = {
		...
		date : {
			...
			month : {
				// 달의 첫째날
				firstDay : function (date, pattern) {
					date = new Date(date.getFullYear(), date.getMonth(), 1);
					return converter.date.basic.toString(date, pattern);
				},
				
				// 달의 마지막날
				lastDay : function (date, pattern) {
...
function getMenuSetting () {
	...
	$.getJSON("http://localhost:8081/mainMenu/menuList", function(data){
		...
		$.each(data.menuList, function(i, item){
			...
			menuList += '	<a href="' + (item.mainMenuName == 'Dashboard' ? item.url : 'javascript:void(0);') + '" class="main_menu_class">';
		...
		$('.sub_menu_li').each(function () {
			...
			// dashboard
			if (thisUrl.indexOf('dashboard') != -1) {
				$('.main_menu_class:eq(0)').addClass('active');
				// body title remove
				$('.this-sm-title-div').remove();
				return false;
			}
